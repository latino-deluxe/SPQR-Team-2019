...
uint8_t BNO055::readByte(uint8_t address, uint8_t subAddress){
	...
	I2c.write(address,subAddress);
	I2c.read(address,(size_t)1);
	data = I2c.receive();
    return data;                             // Return data read from slave register
}

uint8_t BNO055::checkconnection(){
	uint8_t stato = I2c.write(0x28,0x00);
	return stato;
}
...
void BNO055::readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest){
	I2c.write(address,subAddress);
	I2c.read(address,(size_t)count, dest);
}
