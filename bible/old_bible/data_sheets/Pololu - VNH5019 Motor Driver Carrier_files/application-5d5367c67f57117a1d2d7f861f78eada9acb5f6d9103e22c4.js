...
function pictureZoom(event) {
  ...
  lightboxZoom(gallery_id, gallery_data, picture_id)
}
...
function lightboxClickToExit() {
  lightboxGalleryClear()
  lightboxHide()
  history.pushState('', '', window.location.pathname)
}

function lightboxSetFullLink(slide) {
  var url = slide.find('img').attr('src')
  $('#lightbox_full a').attr('href',url)
}

function lightboxZoom(gallery_id, gallery_data, picture_id) {
  if(!gallery_data) return []

  var pictures = JSON.parse(gallery_data)

  lightboxShow()
  lightboxGalleryPrepare(gallery_id, pictures, picture_id)
}

function lightboxSetHash(gallery_id, picture_id) {
  var hash = '#lightbox-picture'+picture_id
  if(gallery_id)
    hash += ';'+gallery_id

  if(lightboxParseHash())
...
function lightboxGetImage() {
  var result = lightboxParseHash()
  ...
  if(result) {
    ...
    if(gallery_id) {
      ...
      var gallery_data = item.attr('data-gallery-pictures')
      lightboxZoom(gallery_id, gallery_data, picture_id)
    }
    else {
      ...
      var gallery_data = item.attr('data-gallery-pictures')
      lightboxZoom(undefined, gallery_data, picture_id)
    }
    ...
    if($('#lightbox_gallery').is(':visible')) { lightboxHide() }
  }
}
...
function lightboxInit() {
  lightboxGalleryInit()
  lightboxGetImage()
  $('#lightbox_gray_screen').click(lightboxClickToExit)
}

function lightboxGalleryChange(event, slick, index) {
  ...
  lightboxSetHash(gallery_id, picture_id)
  lightboxSetFullLink(slide)
  lightboxShowOrHideArrows(slick)
}

function lightboxImg(gallery_id, picture) {
  var img = $('<img/>', {
...
function lightboxGalleryPrepare(gallery_id, pictures, start_picture_id) {
  var container = $('#lightbox_gallery')

  var start_index;
  for(var i=0; i < pictures.length; i++) {
    var picture = pictures[i]
    container.slick('slickAdd',lightboxImg(gallery_id, picture))
  ...
  if(start_index == 0) {
    ...
    lightboxGalleryChange(undefined, container.slick('getSlick'), 0)
  }
}

function lightboxGalleryInit() {
  var container = $('#lightbox_gallery')
  ...
  container.on('afterChange', lightboxGalleryChange)
...
function lightboxShowOrHideArrows(slick) {
...
function lightboxGalleryClear() {
  $('#lightbox_gallery').slick('removeSlide', null, null, true)
}

$(window).hashchange(lightboxGetImage)

$(document).keydown(function(e) {
  if(!$('#lightbox_gallery').is(':visible')) return
